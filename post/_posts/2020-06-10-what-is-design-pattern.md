...
분명 겪었던 문제들과 해결하기 위해 사용했던 방법들이 겹치거나 비슷한 내용들이 있을 것이다. 
나아가서는 오래전 개발자들도 우리가 겪는 문제를 겪었을 것이다.
그래서 그 중 몇몇 개발자들은 이러한 생각을 했다.
> 비슷한 문제를 해결하기 위한 방법을 기억해두고 패턴화 해두면 해당 문제를 만났을 때 잘 해결할 수 있을 것이다.

예를 든 내용을 다시 생각해보면, 일상 생활에 문제를 해결하기 위해 만들었던 `규칙`이 위 내용의 `패턴`이라는 것을 알 수 있다.

그래서 디자인패턴을 위키피디아에서는 아래와 같이 정의하고 있다.
...
위의 내용을 아래와 같이 세분화된 요소로 정의 할 수 있다.
- 컨텍스트(context)
